...
function _ws_meta() {
  if (is_404()) {
    echo '<title>' . __('Page Not Found', '_ws') . (get_option('seo_meta_title') ? ' ' . get_option('seo_meta_title') : '') . '</title>';
    return;
  }
  if (is_search()) {
    echo '<title>' . __('Search Results', '_ws') . (get_option('seo_meta_title') ? ' ' . get_option('seo_meta_title') : '') . '</title>';
    return;
  }
  if (!is_singular()) {
    return;
...
function _ws_build_seo_meta_tags($seo_title, $seo_description, $seo_keywords, $seo_canonical, $seo_no_index, $seo_no_follow, $generic_title, $generic_description) {
  ...
  if (get_post_type() === 'person') {
    ...
    $generic_title = get_the_title() . ($position ? ', ' . $position : '');
    $generic_description = sprintf(__('Learn more about Haley & Aldrich team member %s\'s role in helping clients solve their most pressing environmental and geotechnical engineering challenges.', '_ws'), get_the_title());
  }
  ...
  else if (array_key_exists(get_post_type(), get_option('amp') ?: [])) {
    $html .= '<link rel="amphtml" href="' . get_home_url() . '/amp/' . get_post_type() . '/' . $post->post_name . '/" />';
  }
  else {
